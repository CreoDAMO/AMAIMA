...
# ── Optional: TenSEAL for FHE subsystem ──────────────────────────────────────
# Set FHE_ENABLED=true at build time to install TenSEAL.
# This adds ~2 min to build time. If TenSEAL is unavailable the app starts
# normally — FHE endpoints return 503 rather than crashing.
# Full AVX-512 acceleration requires the 3-stage HEXL+SEAL build (see above).
ARG FHE_ENABLED=false
RUN if [ "$FHE_ENABLED" = "true" ]; then \
